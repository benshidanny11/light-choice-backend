...
import { User, Order, Product } from "../db/models";
  ...
  createNewOrder: async (req, res) => {
    ...
    if (!order) {
      return res
        ...
        .send(getErrorMessage("Could not create order!"));
    }
    ...
    return res.status(STATUSES.CREATED).send({
      ...getSuccessMessage("Order submitted successfully"),
      orderid: order.oid,
    });
  },
    ...
    if (order[0] === 0) {
      return res
        ...
        .send(
          getErrorMessage(
            `Could not ${
              req.body.orderstatus == "APPROVED" ? "approve" : "reject"
            } this order`
          )
        );
    }
      ...
      .send(
        getSuccessMessage(
          `You have successfully ${
            req.body.orderstatus == "APPROVED" ? "approved" : "rejected"
          } order`
        )
      );
  },
    ...
    const orders = await Order.findAll({
      ...
      include: [
        ...
        },
        {
          model: Product,
          as: "product",
          attributes: ["pprice", "pname", "pimage", "pmark"],
        },
      ],
    });
